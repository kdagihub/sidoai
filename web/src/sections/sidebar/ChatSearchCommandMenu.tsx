...
function DynamicFooter() {
  ...
  const actionLabel =
    highlightedItemType === "filter" ? "Tout afficher" : "Ouvrir";

  return (
    <CommandMenu.Footer
      leftActions={
        <>
          <CommandMenu.FooterAction icon={SvgArrowUpDown} label="Choisir" />
...
export default function ChatSearchCommandMenu({
  ...
  const headerFilters = useMemo(() => {
    if (activeFilter === "chats") {
      return [{ id: "chats", label: "Conversations" }];
    }
  ...
  return (
    <>
      <div aria-label="Ouvrir la recherche de chats" onClick={() => setOpen(true)}>
        {trigger}
      </div>
      ...
      <CommandMenu open={open} onOpenChange={handleOpenChange}>
        <CommandMenu.Content>
          <CommandMenu.Header
            placeholder="Rechercher des conversations, des projets..."
            value={searchValue}
          ...
          <CommandMenu.List
            emptyMessage={
              hasSearchValue
                ? "Aucun résultat"
                : "Pas encore de chats ni de projets"
            }
          >
            ...
            {((activeFilter === "chats" && displayedChats.length === 0) ||
              ...
              <TextSeparator text="Fin des résultats" className="mt-auto mb-2" />
            )}
          </CommandMenu.List>
